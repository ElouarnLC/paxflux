...
## Decision
PaxFlux uses local SQLite managed via `better-sqlite3` and Drizzle ORM, configured with the following mandatory PRAGMAs on every connection startup:
```sql
...
2. Backups use the SQLite Online Backup API (`better-sqlite3.backup()`) to produce consistent snapshots without blocking writers or readers.
